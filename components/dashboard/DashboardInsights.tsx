...
function getWorkspaceSignal(stats: DashboardStats) {
  ...
  if (total === 0) {
    return {
      ...
      description:
        "Once you capture a few notes, this space will show you how your priorities are distributed at a glance.",
    };
  }
  ...
  const spread = Math.max(...counts.map((item) => item.value)) - Math.min(...counts.map((item) => item.value));

  if (dominant.value === 0) {
    return {
      title: "Everything is calm and unassigned.",
      description:
        "No notes are currently flagged, which means your workspace is clean and easy to scan.",
    };
  }
  ...
  return {
    ...
    description:
      "That pattern is a good cue to keep the top items visible and use the lower-priority notes as a batching lane.",
  };
}
      ...
      <div className="relative space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <p className={helperTextClassName}>
                A visual breakdown of the workspace so you can spot urgency, balance, and
                room to breathe in seconds.
              </p>
            </div>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(19rem,0.8fr)]">
          <div className="flex h-full flex-col rounded-[2rem] border border-[color:var(--border)] bg-[color:var(--surface-elevated)] p-5 shadow-[0_24px_70px_-44px_rgba(15,23,42,0.2)] backdrop-blur-xl sm:p-6">
            <div className="grid gap-6 lg:grid-cols-[minmax(0,16rem)_minmax(0,1fr)] lg:items-center">
              <div className="relative mx-auto flex w-full max-w-[16rem] items-center justify-center">
                ...
                <div
                  className="relative flex h-52 w-52 items-center justify-center rounded-full shadow-[0_18px_50px_-30px_rgba(15,23,42,0.3)]"
                  style={{
                ...
                >
                  ...
                  <div className="relative z-10 flex h-32 w-32 flex-col items-center justify-center rounded-full border border-[color:var(--border)] bg-[color:var(--surface)] text-center shadow-[0_10px_30px_-20px_rgba(15,23,42,0.22)]">
                    <p className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-[color:var(--muted-foreground)]">
                      Total notes
                    </p>
          ...
          <div className="grid gap-5">
            <div className="rounded-[2rem] border border-[color:var(--border)] bg-[color:var(--surface-elevated)] p-5 shadow-[0_24px_70px_-44px_rgba(15,23,42,0.2)] backdrop-blur-xl sm:p-6">
              ...
              <div className="mt-4 space-y-4">
                {insights.map((item) => {
                  ...
                  return (
                    <div key={item.key} className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-4">
            ...
            <div className="rounded-[2rem] border border-[color:var(--border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(245,247,251,0.95))] p-5 shadow-[0_24px_70px_-44px_rgba(15,23,42,0.2)] backdrop-blur-xl sm:p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--muted-foreground)]">
                Quick read
              </p>
              <div className="mt-4 grid gap-3">
                <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-4">
                  <p className="text-sm font-medium text-[color:var(--foreground)]">
                    {totalNotes > 0
                      ? `${formatPercent(stats.highPriority, totalNotes)}% of notes are high priority.`
                      : "High priority notes will show up here once you add them."}
                  </p>
                </div>
                <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-4">
                  <p className="text-sm font-medium text-[color:var(--foreground)]">
                    {totalNotes > 0
                      ? `${formatPercent(stats.mediumPriority, totalNotes)}% sit in the middle lane.`
                      : "Medium priority notes are great for work you want to batch later."}
                  </p>
                </div>
                <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-4">
                  <p className="text-sm font-medium text-[color:var(--foreground)]">
                    {totalNotes > 0
                      ? `${formatPercent(stats.lowPriority, totalNotes)}% are low priority and easy to revisit.`
                      : "Low priority notes can become your parking lot for ideas."}
                  </p>
                </div>
              </div>
            </div>
